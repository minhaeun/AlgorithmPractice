...
public class SWEA1767 {
    ...
    public static void main(String[] args) throws Exception {
        System.setIn(new FileInputStream("./src/SWEA/SWEA1767/input.txt"));
        ...
        for (int tc = 1; tc <= T; tc++) {
            ...
            map = new int[N][N];
            list = new ArrayList<int[]>();      // 가장 자리가 아닌 코어리스트
            max = 0;  // 최대 연결 코어 수
    ...
    private static void go(int index, int cCnt) {   // 부분집합으로 코어 전선 놓기 시도, cCnt : 전원과 연결된 코어 수
    ...
    private static void setStatus(int r, int c, int d, int s) {   // r, c 위치에[서 d 방향으로 전선을 놓거나(2) 지우거나 (0)
        int nr = r, nc = c;
        while(true){
            nr += dr[d];
            nc += dc[d];
